...
class TeleprompterPiPManager private constructor(private val activity: Activity) {
    ...
    companion object {
        ...
        private const val ACTION_PLAY_PAUSE = "com.cuecard.mobile.PLAY_PAUSE"
        private const val ACTION_STOP = "com.cuecard.mobile.STOP"
    }
    ...
    private fun estimateSegmentDuration(segment: TeleprompterSegment): Int {
        ...
        return (estimatedHeight / defaultScrollSpeed).toInt()
    }
    ...
    private fun renderSegmentText(canvas: Canvas, text: String, x: Float, y: Float, maxWidth: Float) {
        ...
        val matcher = notePattern.matcher(text)
        ...
        // Apply pink color to [note ...] sections (in reverse order to maintain positions)
        for ((start, end, _) in matches.reversed()) {
        ...
        val lines = spannableBuilder.toString().split("\n")
        var currentY = y
